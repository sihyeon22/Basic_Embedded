...
void TIM4_Repeat_Interrupt_Enable(int en, int time)
{
	if(en)
	{
		...
		Macro_Clear_Bit(TIM4->SR, 0);	// TIM4->SR 레지스터에서 Timer Pending Clear
		NVIC_ClearPendingIRQ(30);		// NVIC에서 30번 인터럽트 Pending Clear => NVIC용 Macro 사용

		Macro_Set_Bit(TIM4->DIER, 0);	// TIM4->DIER 레지스터에서 Timer 인터럽트 허용
		NVIC_EnableIRQ(30);				// NVIC에서 30번 인터럽트를 허용으로 설정 => NVIC용 Macro 사용
