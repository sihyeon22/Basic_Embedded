...
#define mTIM2_FREQ 	  		(8000000.) 	      		// Hz
#define mTIM2_TICK	  		(1000000./mTIM2_FREQ)	// usec
#define mTIME2_PLS_OF_1ms  	(1000./mTIM2_TICK)
...
void TIM2_Motor_Out_Init(void)
{
	...
	TIM2->CCER = (0<<13)|(0<<12)|(0<<9)|(0<<8); // PA2, PA3 out disable
}
...
void TIM2_Motor_Run(int dir, float duty) // dir => -1, 0, 1
{
	...
	switch(dir)
	{
		case 1 : // cw
			TIM2->CCER &= ~(1<<12);
			TIM2->CCER |= (1<<8); // PA2 = ON
			break;
		case -1 : // ccw
			TIM2->CCER &= ~(1<<8);
			TIM2->CCER |= (1<<12); // PA3 = on
			break;
		case 0 : // stop
			TIM2->CCER &= ~(1<<8);
			TIM2->CCER &= ~(1<<12);
